...
const {SendPickupToReciverDetails} = require("./SendEmailToReciverPickup")
exports.createPickupDetails=async (req,res)=>{
    try {
        let count = (await PickupDetails.countDocuments()+1)+1000;
        const result1 =  await PickupDetails.create({
            Date:req.body.Date,
            Time:req.body.Time ,
            RequestedId:count,
           ...
           if(result1){
            const result = await PickupDetails.aggregate([
                ...
                $match:{ReceiverEmail:result1.ReceiverEmail},
  ...
  ${result[0].Customer[0].Mobile}
        ...
        if(result1){
            const result = await PickupDetails.aggregate([
                ...
                {
                    $match:  {SenderEmail:result1.SenderEmail},
                }
            ])  
            var  transporter = nodemailer.createTransport({
                service: "gmail",
